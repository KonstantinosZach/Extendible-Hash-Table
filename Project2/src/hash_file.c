...
int store_record(Record record, char* data){
  ...
  memcpy(data + offset, id_string, INT_SIZE);
  offset += sizeof(char)*INT_SIZE;
  memcpy(data + offset, record.name, NAME_SIZE);
  offset += sizeof(char)*NAME_SIZE;
  memcpy(data + offset, record.surname, SURNAME_SIZE);
  offset += sizeof(char)*SURNAME_SIZE;
  memcpy(data + offset, record.city, CITY_SIZE);
...
void split(int index, int old_pointer, char* bucket, Record record, char* dir, int* tuple_id,UpdateRecordArray updateArray[MAX_RECORDS]){
  ...
  //ενημερώνουμε ότι χρειάζεται να κληθεί η SHT_SecondaryUpdateEntry
  updateflag++;
  ...
  //Βάζουμε και στο overflowed bucket 0 counter
  ...
  for(int rec=0; rec<MAX_RECORDS; rec++){
    ...
    if(check_depth == 0){
      ...
      //ενημερώνεται ο πίνακας με τα records που πρέπει να τους αλλάξουμε τα tuple id 
      //καθώς μετακινήθηκαν από την παλιά τους θέση στο πρωτεύον ευρετήριο
      ...
      memcpy(new_block_data + new_offset,string_id,INT_SIZE);

      new_offset += INT_SIZE;
      memcpy(new_block_data + new_offset,name,NAME_SIZE);

      new_offset += NAME_SIZE;
      memcpy(new_block_data + new_offset,surname,SURNAME_SIZE);

      new_offset += SURNAME_SIZE;
      memcpy(new_block_data + new_offset,city,CITY_SIZE);
      ...
      memcpy(bucket + old_offset,string_id,INT_SIZE);

      old_offset += INT_SIZE;
      memcpy(bucket + old_offset,name,NAME_SIZE);

      old_offset += NAME_SIZE;
      memcpy(bucket + old_offset,surname,SURNAME_SIZE);

      old_offset += SURNAME_SIZE;
      memcpy(bucket + old_offset,city,CITY_SIZE);
    ...
    if(rec == MAX_RECORDS - 1){
      ...
      //ενημερώνουμε το tuple id της καινούργιας εγγραφής που προκάλεσε το split
      //καθώς το tuple idd που του είχε δωθεί στην insert πλέον δεν ισχύει
      ...
      if(store_record(record,new_bucket) == -1){
...
void pointers_adapt(int new_depth, char* dir, int overflowed_bucket, int last){
  ...
  for(int i=0; i<pow(2,new_depth); i++){
    ...
    if(pointer == overflowed_bucket && flag==0){
      ...
      memcpy(dir + bit_offset, string_last, INT_SIZE);
      free(string_last);
      flag++;
    }
...
HT_ErrorCode HT_InsertEntry(int indexDesc, Record record, int* tupleId, UpdateRecordArray* updateArray) {
  ...
  //το tuple id που χρειάζεται στο δευτερεύον ευρετήριο
  *tupleId = (pointer)*MAX_RECORDS + index_of_rec_in_block;
